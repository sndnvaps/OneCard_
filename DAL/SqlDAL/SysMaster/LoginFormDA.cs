...
namespace DAL.SqlDAL.SysMaster
{
    class LoginFormDA : ILoginFormDA
    {
        private Sys_UserMaster_usm_Info FindForm(Sys_UserMaster_usm_Info info)
        {
            ...
            sqlString += "SELECT distinct fom_iRecordID, fom_iParentID, fom_iIndex, fom_cFormNumber, fom_cFormDesc, fom_cExePath " + Environment.NewLine;
            ...
            sqlRole += "SELECT distinct fom_iRecordID, fom_iParentID, fom_iIndex, fom_cFormNumber, fom_cFormDesc, fom_cExePath " + Environment.NewLine;
                ...
                using (SIOTSDB_HHZXDataContext db = new SIOTSDB_HHZXDataContext())
                {
                    IQueryable<Sys_UserMaster_usm> taQuery =
                        (from ta in db.Sys_UserMaster_usm
                         where ta.usm_cUserLoginID == usm.usm_cUserLoginID && ta.usm_cPwd == usm.usm_cPwd
