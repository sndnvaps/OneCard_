...
namespace WinCA.MealListUpload
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                MealWhiteListUploadService serviceW = new MealWhiteListUploadService();
                serviceW.Upload();
                return;

                _LocalLogger = new Common.General(Common.General.BindLocalLogInfo());

                Console.WriteLine(getCWStyle("用餐名单上传服务启动", SystemLog.SystemLog.LogType.Trace));
                ...
                MealListUploadService service = new MealListUploadService();
                service.LocalLogger = _LocalLogger;
                service.ErrRetryTimes = iRetryTimes;
                service.ErrRetryInterval = iRetryInterval;

                bool blnRes = service.Upload();
                if (blnRes)
                    Console.WriteLine(getCWStyle("用餐名单上传完毕。", SystemLog.SystemLog.LogType.Trace));
                else
                {
                    Console.WriteLine(getCWStyle("用餐名单上传失败", SystemLog.SystemLog.LogType.Trace));
